...
struct MmoDataDumper {
    ...
    port: u16,
    #[arg(short, long, help = "Combine mmo.bin and mo.bin into one file (combo.bin).")]
    combined: bool,
}
...
async fn main() {
    ...
    if let Ok(mut client) = SysBotClient::connect(&dumper_args.ip, dumper_args.port).await {
        if let Ok(mo_data) = client.pointer_peek(&[0x42BA6B0, 0x2B0, 0x58, 0x18, 0x20], 0x190).await {
            if !dumper_args.combined {
                let file = OpenOptions::new().write(true).create(true).truncate(true).open("mo.bin");
                if let Ok(mut file) = file {
                    file.write_all(&mo_data[..(mo_data.len() - 1)]).unwrap();
                }
            }

            if let Ok(mmo_data) = client.pointer_peek(&[0x42BA6B0, 0x2B0, 0x58, 0x18, 0x1B0], 0x3980).await {
                if !dumper_args.combined {
                    let file = OpenOptions::new().write(true).create(true).truncate(true).open("mmo.bin");
                    if let Ok(mut file) = file {
                        file.write_all(&mmo_data[..(mmo_data.len() - 1)]).unwrap();
                    }
                } else {
                    let file = OpenOptions::new().write(true).create(true).truncate(true).open("combo.bin");
                    if let Ok(mut file) = file {
                        file.write_all(&mo_data[..(mo_data.len() - 1)]).unwrap();
                        file.write_all(&mmo_data[..(mmo_data.len() - 1)]).unwrap();
                    }
